...
router.get("/public/leaderboard/:code", async (req, res) => {
  try {
    const test = await Test.findOne({
      where: { access_code: req.params.code },
    });
...
router.get("/public/:code", async (req, res) => {
  ...
  try {
    ...
    if (!test)
      return res
        .status(404)
        .json({ error: "Test nie istnieje lub kod jest błędny." });
    if (!test.is_public)
      return res.status(403).json({ error: "Ten test jest prywatny." });
...
router.get("/:id/stats", requireAuth, async (req, res) => {
  ...
  try {
      ...
      include: [
        {
          ...
          include: [
            {
              model: Question,
              attributes: ["text", "question_type", "points"],
            },
          ],
        },
      ],
    ...
    const totalMaxPoints = test.Questions.reduce(
    ...
    const avgScore =
      count > 0 ? (scores.reduce((a, b) => a + b, 0) / count).toFixed(1) : 0;
...
router.put("/:id", requireAuth, async (req, res) => {
  const user_id = req.session.userId;
  const { id } = req.params;
  const { title, description, is_public, scoringMethod, scoreThresholds } =
    req.body;
