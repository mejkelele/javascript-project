...
const testData = ref({
  ...
  is_public: true,
  show_answers: true,
  attempts_limit: 0 // 0 = brak limitu
})
...
onMounted(async () => {
    ...
    if (isEditMode.value) {
        ...
        try {
            ...
            // Wczytanie limitu
            if (testData.value.attempts_limit === undefined) testData.value.attempts_limit = 0;
  ...
  <div class="create-page">
    ...
    <div class="form-section">
      ...
      <div class="settings-grid">
        ...
        <div class="limit-wrapper">
            <label for="limit" style="margin-top:0">Limit podejść:</label>
            <input type="number" id="limit" v-model="testData.attempts_limit" min="0" class="limit-input" />
            <span class="hint-small">(0 = brak limitu)</span>
        </div>
      </div>
    ...
    <section class="scoring-section">
      ...
      <select v-model="scoringMethod">
        <option value="standard">Domyślna (2.0 → 5.0)</option>
    ...
    <div class="form-section">
      ...
      <div v-for="(question, qIndex) in questions" :key="qIndex" class="question-card">
        ...
        <div v-else-if="question.question_type === 'FILL'" class="fill-box">
            <p class="sub-label">Poprawna odpowiedź:</p>
        ...
        <div v-else-if="question.question_type === 'OPEN'" class="open-box">
            <p class="info-text">ℹ️ Pytanie otwarte.</p>
        </div>
      </div>
...
/* Nowe style */
.settings-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 15px; align-items: start; }
...
.limit-wrapper { display: flex; flex-direction: column; }
.limit-input { width: 80px; }
.hint-small { font-size: 0.8rem; color: var(--color-text-light-2); }
</style>